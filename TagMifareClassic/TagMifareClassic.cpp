...
TagMifareClassic::TagMifareClassic(Reader *reader)
        : Tag(reader), tagType(MIFARE_UNKNOWN), uid({}), supportsAnticollision(false), state(POWER_OFF), keyType(KEY_A), key(NULL), sectorTrailerProtected(
        true) {
}

TagMifareClassic::Uid TagMifareClassic::getUid() {
    return uid;
}

bool TagMifareClassic::hasAnticollisionSupport() {
    return supportsAnticollision;
}

TagMifareClassic::TagType TagMifareClassic::getTagType() {
    return tagType;
}

void TagMifareClassic::setState(TagMifareClassic::State state) {
    this->state = state;
}

TagMifareClassic::State TagMifareClassic::getState() {
    return state;
}

bool TagMifareClassic::activate() {
    return request() && hasAnticollisionSupport() && select();
}

bool TagMifareClassic::activateWakeUp() {
    return wakeUp() && hasAnticollisionSupport() && select();
}

bool TagMifareClassic::detect(unsigned char command) {
...
bool TagMifareClassic::request() {
    return detect(REQUEST);
}

bool TagMifareClassic::wakeUp() {
    return detect(WAKE_UP);
}

bool TagMifareClassic::select() {
    if (getState() != READY) {
        return false;
    }
...
bool TagMifareClassic::authenticate(unsigned char address, KeyType type, unsigned char *key) {
...
bool TagMifareClassic::readBlock(unsigned char address, unsigned char *buf) {
...
bool TagMifareClassic::writeBlock(unsigned char address, unsigned char *buf) {
...
bool TagMifareClassic::readBlockSlice(unsigned char address, unsigned char from, unsigned char len, unsigned char *buf) {
...
bool TagMifareClassic::writeBlockSlice(unsigned char address, unsigned char from, unsigned char len, unsigned char *buf) {
    unsigned char receive[18];
...
int TagMifareClassic::readByte(unsigned char address, unsigned char pos) {
...
bool TagMifareClassic::writeByte(unsigned char address, unsigned char pos, unsigned char value) {
    unsigned char buf[18];
    if (!readBlock(address, buf)) {
        return false;
    }
...
bool TagMifareClassic::increment(unsigned char address, uint32_t delta) {
    return arithmeticOperation(INCREMENT, address, delta);
}

bool TagMifareClassic::decrement(unsigned char address, uint32_t delta) {
    return arithmeticOperation(DECREMENT, address, delta);
}

bool TagMifareClassic::restore(unsigned char address) {
    return arithmeticOperation(RESTORE, address, 0);
}

bool TagMifareClassic::arithmeticOperation(unsigned char operation, unsigned char address, uint32_t delta) {
...
bool TagMifareClassic::transfer(unsigned char address) {
...
bool TagMifareClassic::createValueBlock(unsigned char address, uint32_t value, uint8_t addr) {
    Access access;
    unsigned char buf[18];
...
bool TagMifareClassic::readAccessBits(unsigned char sector, unsigned char *accessBits) {
    return readBlockSlice(getSectorTrailerAddress(sector), TAG_ACCESS_POSITION, TAG_ACCESS_BITS_SIZE, accessBits);
}

bool TagMifareClassic::writeAccessBits(unsigned char sector, unsigned char *accessBits, unsigned char *keyA, unsigned char *keyB) {
    unsigned char buf[18];
...
bool TagMifareClassic::setAccessCondition(unsigned char sector, unsigned char block, Access access, unsigned char *keyA, unsigned char *keyB) {
...
bool TagMifareClassic::getAccessCondition(unsigned char address, Access *access) {
...
bool TagMifareClassic::writeKey(unsigned char sector, KeyType type, unsigned char *keyA, unsigned char *keyB) {
    unsigned char buf[18];
...
bool TagMifareClassic::readKey(unsigned char sector, KeyType type, unsigned char *key) {
    unsigned from = TAG_KEY_TYPE_TO_POS(type);
    return readBlockSlice(getSectorTrailerAddress(sector), from, TAG_KEY_SIZE, key);
}

void TagMifareClassic::setupAuthenticationKey(KeyType keyType, unsigned char *key) {
    this->keyType = keyType;
    this->key = key;
}

unsigned char TagMifareClassic::computeNvb(unsigned char collisionPos) {
    unsigned char bytes = collisionPos / 8;
    unsigned char bits = collisionPos % 8;
    return (((bytes << 4) & 0xf0) | (bits & 0x0f)) + 0x20;
}

bool TagMifareClassic::isAccessBitsCorrect(unsigned char *accessBits) {
    unsigned char *c = accessBits;
    return (~(c[1] >> 4) & 0x0f) == (c[0] & 0x0f) && (~(c[2]) & 0x0f) == ((c[0] >> 4) & 0x0f) && (~(c[2] >> 4) & 0x0f) == (c[1] & 0x0f);
}

void TagMifareClassic::computeTagType() {
    switch (uid.sak & 0x7f) {
    case 0x04:
...
void TagMifareClassic::packAccessBits(unsigned char *accessBits, unsigned char c1, unsigned char c2, unsigned char c3) {
    accessBits[0] = (~(c2 << 4) & 0xf0) | ((~c1) & 0x0f);
    accessBits[1] = ((c1 << 4) & 0xf0) | ((~c3) & 0x0f);
    accessBits[2] = ((c3 << 4) & 0xf0) | (c2 & 0x0f);
}

void TagMifareClassic::unpackAccessBits(unsigned char *accessBits, unsigned char *c1, unsigned char *c2, unsigned char *c3) {
    *c1 = (accessBits[1] >> 4) & 0x0f;
    *c2 = accessBits[2] & 0x0f;
    *c3 = (accessBits[2] >> 4) & 0x0f;
}

void TagMifareClassic::fillValueBlock(unsigned char *buf, uint32_t value, uint8_t addr) {
...
void TagMifareClassic::setSectorTrailerProtected(bool protect) {
